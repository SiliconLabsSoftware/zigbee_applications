 ...
 *
 * @param arguments CLI arguments following form
 */
void event_network_form_handler(sl_cli_command_arg_t *arguments)
{
  ...
  if (state != SL_ZIGBEE_JOINED_NETWORK) {
    ...
    sl_zigbee_af_core_println("%p network %p: 0x%X", "Form", "start", status);
  } else {
 ...
 *
 * @param arguments CLI arguments following open
 */

...
sl_zigbee_af_event_t event_rejoin_ctrl;
...
void my_join_handler(sl_cli_command_arg_t *arguments)
{
  ...
  if (status == SL_STATUS_OK) {
    sl_zigbee_af_debug_print("Start of Network Steering successful");
  } else {
    sl_zigbee_af_debug_print("ERROR to start Network Steering");
  }
}
 ...
 */
void my_event_rejoin_handler(sl_zigbee_event_context_t *context)
{
  sl_status_t status;

  status = sl_zigbee_find_and_rejoin_network(true,
                                             SL_ZIGBEE_ALL_802_15_4_CHANNELS_MASK,
                                             SL_ZIGBEE_REJOIN_REASON_NONE,
                                             SL_ZIGBEE_SLEEPY_END_DEVICE);
  if (status == SL_STATUS_OK) {
    sl_zigbee_af_debug_print("Succeeded to launch Secured Rejoining");
  } else {
    sl_zigbee_af_debug_print("Failed to launch Secured Rejoining");
  }
}

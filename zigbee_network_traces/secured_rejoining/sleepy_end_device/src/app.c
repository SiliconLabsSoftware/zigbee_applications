 ...
 *
 * @param status On success this will be set to  to indicate a
                                               ...
                                               uint8_t finalState)
{
  ...
  if (status == SL_STATUS_OK) {
    sl_zigbee_af_debug_print("Launching Rejoin in 15 seconds");
    sl_zigbee_af_event_set_delay_ms(&event_rejoin_ctrl, REJOIN_DELAY_MS);
  } else {
    sl_zigbee_af_debug_print("Failed to join a network, rejoin aborted");
  }
}

/** @brief
 *
 * Application framework equivalent of ::sl_zigbee_af_radio_needs_calibrating_cb
 */
...
void sl_zigbee_af_main_init_cb(void)
{
  ...
  sl_zigbee_af_event_init(&event_rejoin_ctrl, my_event_rejoin_handler);

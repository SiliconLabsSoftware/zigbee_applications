                                       ...
                                       sl_zigbee_join_decision_t decision)
{
  ...
  sl_zigbee_af_debug_print("Child 0x%x%x has %s the channel",
                           ((newNodeId >> 8) & 0xff),
                           (newNodeId & 0xff),
                           ((status)
  ...
  if (status != SL_ZIGBEE_DEVICE_LEFT) {
    ...
    sl_zigbee_af_debug_print("Current NodeID : 0x%x%x", (newNodeId >> 8) & 0xff,
                             (newNodeId & 0xff));
    current_index += 1;
  }
  sl_zigbee_af_debug_print(
...
void event_network_form_handler(sl_cli_command_arg_t *arguments)
{
  ...
  sl_zigbee_af_debug_print("Network formation");
...
void network_open_handler(sl_cli_command_arg_t *arguments)
{
  ...
  sl_zigbee_af_debug_print("Open Network with well-Known key");
  state = sl_zigbee_af_network_state();

  // Check if Network Created
  if (state != SL_ZIGBEE_JOINED_NETWORK) {
    sl_zigbee_af_debug_print("Network not Joined, cannot open the network");
  } else {
    sl_zigbee_af_debug_print("Network UP, opening process launched");
    status = sl_zigbee_af_network_creator_security_open_network();
    UNUSED_VAR(status);
    sl_zigbee_af_debug_print("Network Open with Key : 0x%X", status);
  }
}

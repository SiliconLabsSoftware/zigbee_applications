...
#endif \
  // SL_CATALOG_SIMPLE_BUTTON_PRESENT && SL_ZIGBEE_APP_FRAMEWORK_USE_BUTTON_TO_STAY_AWAKE == 0

#include "trust_center_swap_out_callbacks.h" // TCSO features
...
void sl_zigbee_af_main_init_cb(void)
{
  ...
  // To add the custom TCSO commands into CLI table
  sl_zigbee_app_debug_println("Add custom CLI group: %s",
                              trust_center_swap_out_cli_register() ? "Success"
                              : "Failed");
}
  ...
  if (status != SL_STATUS_OK) {
    ...
    if (status != SL_STATUS_OK) {
      sl_zigbee_app_debug_println(
        "Error: cannot initialize ZLL security: 0x%02X",
        status);
    }
#endif // SL_CATALOG_ZIGBEE_ZLL_COMMISSIONING_COMMON_PRESENT
    status = sl_zigbee_af_network_creator_start(true); // To form centralized network
